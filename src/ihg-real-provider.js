...
function normalizePayload(payload) {
  ...
  return {
    ...
    prices: prices.map((price) => ({
      ...
      rawPayload: price,
    })).filter((price) => price.providerHotelId && price.checkinDate),
  };
}
...
async function fetchIhgRealData() {
  if (process.env.IHG_SYNC_SOURCE_URL) {
    return normalizePayload(await fetchJsonFromUrl(process.env.IHG_SYNC_SOURCE_URL));
  }
